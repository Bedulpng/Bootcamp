...
export function useNotifications(initialNotifications: Notification[] = []) {
  ...
  useEffect(() => {
    ...
    const socket = io('http://10.10.103.160:4000', {
  ...
  const markAsRead = useCallback(async (id: string) => {
    try {
      await axios.put(`http://10.10.103.160:4000/mentor/notification/${id}/read`);
      setNotifications((prev) => prev.filter((notif) => notif.id !== id));
    } catch (error) {
  ...
  const markAllAsRead = useCallback(async () => {
    try {
      ...
      await axios.put('http://10.10.103.160:4000/mentor/notifications/read-all');
