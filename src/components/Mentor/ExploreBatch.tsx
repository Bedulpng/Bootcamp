  ...
  useEffect(() => {
    ...
    const fetchMentorBatches = async () => {
      try {
        ...
        const response = await axios.get(`http://10.10.103.10:4000/admin/batch/${mentorId}`); // Replace with your API URL
      ...
      {(activeFilter === 'all-batch' ? batches : mentorBatches).map((batch) => (
          ...
          onClick={async () => {
            try {
              ...
              const response = await fetch(`http://10.10.103.10:4000/admin/batchs/${batch.id}`);
              if (!response.ok) {
