...
const NavbarTrainee: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMentorDetails = async () => {
      try {
        ...
        const response = await axios.get(`http://192.168.181.104:4000/admin/mentor/${userId}`);
        setTraineeName(response.data.fullName);
        setTraineeRole(response.data.role);

        // Fetch the professional profile image
        const profileResponse = await axios.get(`http://192.168.181.104:4000/trainee/${userId}/pro`);
  ...
  const confirmLogout = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.181.104:4000/trainee/logout', {
        accessToken,
      });
            ...
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="flex items-center gap-2 p-1">
                <Avatar className="h-10 w-10 border-2 border-gray-200 rounded-full">
                  <AvatarImage src={`http://192.168.181.104:4000${profileImage}`} alt="Trainee" />
                  <AvatarFallback>
                    {traineeName
                      ? traineeName
                          .split(' ')
