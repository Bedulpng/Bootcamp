...
export const fetchBatches = async (): Promise<Batch[]> => {
  ...
  export const fetchBatchesById = async (batchId : string): Promise<Batch[]> => {
    try {
      const response = await axios.get(`http://192.168.1.7:4000/admin/batch/${batchId}`);
      return response.data;
    } catch (error) {
      console.error('Error fetching batches:', error);
      throw error;
  ...
  export const editBatch = async (
    ...
    try {
      const response = await axios.put(
        `http://192.168.1.7:4000/admin/batch/${id}`,
        batchPayload
      );
      return response.data;
    } catch (error) {
  ...
  export const assignParticipantsToBatch = async (
    ...
    try {
      const response = await axios.put(
        `http://192.168.1.7:4000/admin/batch/${batchId}/participants`,
        { participants }
      );
      return response.data;
    } catch (error) {
...
export const fetchClasses = async (): Promise<Class[]> => {
  try {
      const response = await axios.get('http://192.168.1.7:4000/admin/class');
      return response.data;
    } catch (error) {
  ...
  export const fetchCB = async (userId: string): Promise<{ classes: Class[], batches: Batch[] }> => {
    try {
      const response = await axios.get(`http://192.168.1.7:4000/trainee/${userId}/cb`);
      return response.data;
    } catch (error) {
