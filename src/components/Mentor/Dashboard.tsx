...
export default function MentorDb() {
  ...
  const [batch, setMyBatch] = useState<number[] | null>(null); // Allow null state
    ...
    const fetchScheduledClasses = async () => {
      try {
        ...
        const response = await fetch(
          "http://10.10.103.204:4000/mentor/schedule",
          {
            method: "GET",
            headers: {
    ...
    const fetchBatches = async () => {
      try {
        const response = await axios.get(
          "http://10.10.103.204:4000/admin/batch"
  ...
  useEffect(() => {
    ...
    const fetchBatches = async () => {
      try {
        ...
        const response = await axios.get(
          `http://10.10.103.204:4000/admin/batch/${mentorId}`
        );
        ...
        const batchNumbers = response.data.map(
          (batch: { batchNum: number }) => batch.batchNum
        );
        setMyBatch(batchNumbers);
      } catch (error) {
        console.error("Error fetching batches:", error);
        setMyBatch(null); // Handle errors by setting to null
      }
    };
  ...
  useEffect(() => {
    const fetchCompletionPercentages = async () => {
      try {
        ...
        const response = await axios.get(
          "http://10.10.103.204:4000/mentor/completion-percentage",
          {
            headers: {
        ...
        const { wholeClassLessonPercentage, wholeClassChallengePercentage } =
          response.data;
  ...
  useEffect(() => {
    const fetchMentorDetails = async () => {
      try {
        ...
        const response = await axios.get(
          `http://10.10.103.204:4000/admin/mentor/${userId}`
        );
        ...
        const profileResponse = await axios.get(
          `http://10.10.103.204:4000/trainee/${userId}/pro`
        );
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        const response = await axios.get(
          "http://10.10.103.204:4000/admin/class"
        ); // Replace with your API URL
                              ...
                              scheduledClasses
                                ...
                                .map((scheduledClass, index) => (
                                  <div key={index} className="mt-6">
                                    ...
                                    {Array.isArray(
                                      scheduledClass.scheduleDays
                                    ) &&
                                    scheduledClass.scheduleDays.length > 0 ? (
                                      scheduledClass.scheduleDays
                                        .slice(0, 2)
                                        .map(
                                          (
                        ...
                        <div className="flex flex-col gap-4 w-36">
                          <div className="bg-white rounded-xl h-48 flex flex-col overflow-hidden">
                            ...
                            >
                              ...
                              {batch && batch[currentIndex] ? batch[currentIndex] : "N/A"}
                              </div>
                            </div>
                            {batch && batch.length > 1 ? (
                              ...
                              <div className="mt-auto px-6 pb-6">
                                ...
                                {/* Show a message when batch is null */}
                              </div>
                            ) : null}
                          </div>
                          <Button
                  ...
                  <Card className="bg-white rounded-2xl shadow-lg">
                    <CardContent className="p-6">
                      <h3 className="font-bold text-xl mb-1">
                        Trainee login activity
                      </h3>
                      <p className="text-sm text-gray-500">
                        Trainee Logged in today
                      </p>
                    ...
                    <div className="text-center flex-1 flex flex-col items-center justify-center">
                      <div className="w-17 h-17 rounded-full border-2 border-gray-200 flex items-center justify-center mb-6">
                        <Avatar className="h-16 w-16 border-gray-800 rounded-full">
                          <AvatarImage
                            src={`http://10.10.103.204:4000${profileImage}`}
                            alt="Mentor"
                          />
                          <AvatarFallback>
                            {mentorName
