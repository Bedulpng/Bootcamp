...
export default function PresentationUpload({ onSuccess, classId, batchId }: presentationUploadFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch("http://10.10.103.248:4000/uploads/presentation", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        setError(errorData.error || "Failed to upload the presentation.");
        return;
      }
