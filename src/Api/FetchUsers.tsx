...
export async function fetchUsers(): Promise<Trainee[]> {
  try {
    const response = await axios.get('http://10.10.103.248:4000/admin/users');
...
export async function fetchNotesByGrader(graderId: string, visibility: string): Promise<Note[]> {
  try {
    ...
    const response = await axios.get<Note[]>(
      `http://10.10.103.248:4000/mentor/notes/${graderId}/${visibility}`,
      {
        headers: {
...
export async function fetchClassesByUserId(userId: string): Promise<Class[]> {
  try {    
    const response = await axios.get<Class[]>(
      `http://10.10.103.248:4000/trainee/class/user/${userId}`, // Replace with your actual API URL
      // {
      //   headers: {
      //     Authorization: `Bearer ${token}`,
      //   },
