...
export async function fetchCertificate(userId: string): Promise<string | null> {
  try {
    const response = await axios.get(`http://10.10.103.25:4000/trainee/${userId}/certificate`);
...
export async function fetchAllCertificates(): Promise<any[]> {
  try {
    const response = await axios.get("http://10.10.103.25:4000/trainee/certificates");
...
export async function fetchCertificateById(certificateId: string): Promise<any | null> {
  try {
    const response = await axios.get(`http://10.10.103.25:4000/trainee/certificates/${certificateId}`);
    return response.data;
  } catch (error) {
